...
上面的示例展示了闭包的基本实现方式。在 `outer_function` 中定义了 `inner_function`，并返回了 `inner_function`。当调用 `outer_function(5)` 后，它返回 `inner_function` 的引用，并捕获了 `outer_var` 的值为 5。这就形成了一个闭包。
